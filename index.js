...
// Connect function that can be awaited
...
app.get("/admin/recipe-reports", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.status(500).send({ message: "Error fetching recipe reports", error });
  }
});
...
app.get("/featured-recipes", async (req, res) => {
  try {
    await connectDB();
    const result = await recipesCollection.find({ isFeatured: true }).toArray();
    res.send(result);
  } catch (error) {
